...
class StrokeDataset(Dataset):
    def __init__(self, strokes, texts, chars, max_seq_length=1100, max_text_length=50, name='', augment=False):
        ...
        self.x_bins = np.linspace(0, 4, 101)

        y_bins_pen_down = np.linspace(-1, 1.1, 101)
        y_bins_pen_up = y_bins_pen_down + max(y_bins_pen_down) + 1  # Offset for pen-up states
        self.y_bins = np.concatenate([y_bins_pen_down, y_bins_pen_up])

        self.feature_sizes = [len(self.x_bins), len(self.y_bins)]
    ...
    def encode_stroke(self, stroke):
        x_idx = np.digitize(stroke[:, 0], self.x_bins) - 1
        y_idx = np.digitize(stroke[:, 1], self.y_bins[:len(self.y_bins)//2]) - 1
        y_idx[stroke[:, 2] == 0] += len(self.y_bins) // 2  # Offset for pen-up states

        encoded = np.column_stack([
            x_idx + self.cumulative_sizes[0],
            y_idx + self.cumulative_sizes[1]])
    ...
    def decode_stroke(self, ix):
        ...
        x_idx = ix[:, 0] - self.cumulative_sizes[0]
        assert x_idx < len(self.x_bins), f'Something is wrong: x_idx.max() = {x_idx.max()}'
        x = self.x_bins[x_idx]

        y_idx = ix[:, 1] - self.cumulative_sizes[1]
        pen = (y_idx < len(self.y_bins) // 2).astype(int)
        y_idx[pen == 0] -= len(self.y_bins) // 2
        assert y_idx.max() < len(self.y_bins) // 2, f'Something is wrong: y_idx.max() = {y_idx.max()}'
        y = self.y_bins[:len(self.y_bins)//2][y_idx]

        return np.column_stack([x, y, pen])
