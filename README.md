...
**Better model.** We increased the dataset size to 500k examples including both print and cursive examples. Then we trained for 50k steps on an A100 (instead of 30k on an L4, which was still taking much too long). We experimented with a learning rate schedule but it appeared best to keep the learning rate high (3e-3) in this regime. Context window is larger too with a size of 1500 tokens. Test set samples:
