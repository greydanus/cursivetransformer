...
This tokenization scheme is completely ad hoc and there may be better/simpler/more efficient ways to do this. Whatever the case, now that we have tokens that look just like those used for language modeling, we can use boilerplate LLM Transformer code from hereon out.
...
I decided to draw out one word at a time and then computationally stitch them together to create combinatorially many 3-word sequences. Thus from a few hundred words I was able to bootstap a dataset of tens of thousands of examples.

**Fixing cross attention.** I fixed a major bug in cross attention. I had forgotten to add a positional embedding to the character-level token embedding for the cross-attention layer. Upon fixing this, sample quality improved dramatically. Surprising that the model was able to generate half-coherent sequences before this. Increasing learning rate by 10x made training a lot faster.

![sample_v3](static/sample_v3.png)
